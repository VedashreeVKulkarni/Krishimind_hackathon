...
const buildExplanation = (d, lang) => {
  ...
  if (lang === "hi") {
    if (action === "HOLD") return `${hi} का भाव अगले ${days} दिनों में ₹${current} से बढ़कर ₹${price}/क्विंटल होने का अनुमान है (+${pct}%)। बाजार में तेजी है — अभी मत बेचें। नीचे बताए सरकारी गोदाम में रखें और ज़्यादा मुनाफे में बेचें। भरोसेमंदी: ${conf}/100।`;
    if (action === "SELL") return `${hi} का भाव अगले ${days} दिनों में ₹${current} से घटकर ₹${price}/क्विंटल हो सकता है (-${pct}%)। बाजार में मंदी है — जल्द बेच दें, देर से नुकसान होगा। भरोसेमंदी: ${conf}/100।`;
    return `${hi} का भाव अगले ${days} दिनों में ₹${price}/क्विंटल के आसपास स्थिर रहेगा। बाजार सामान्य है — जल्दबाजी न करें। भरोसेमंदी: ${conf}/100।`;
  }
  if (lang === "kn") {
    if (action === "HOLD") return `${kn} ಬೆಲೆ ಮುಂದಿನ ${days} ದಿನಗಳಲ್ಲಿ ₹${current} ರಿಂದ ₹${price}/ಕ್ವಿಂಟಲ್‌ಗೆ ಏರಬಹುದು (+${pct}%). ಮಾರುಕಟ್ಟೆ ಏರಿಕೆಯಲ್ಲಿದೆ — ಈಗ ಮಾರಬೇಡಿ. ಕೆಳಗೆ ತೋರಿಸಿದ ಸರ್ಕಾರಿ ಗೋದಾಮಿನಲ್ಲಿ ಇರಿಸಿ ಹೆಚ್ಚು ಲಾಭ ಪಡೆಯಿರಿ. ವಿಶ್ವಾಸಾರ್ಹತೆ: ${conf}/100.`;
    if (action === "SELL") return `${kn} ಬೆಲೆ ಮುಂದಿನ ${days} ದಿನಗಳಲ್ಲಿ ₹${current} ರಿಂದ ₹${price}/ಕ್ವಿಂಟಲ್‌ಗೆ ಇಳಿಯಬಹುದು (-${pct}%). ಮಾರುಕಟ್ಟೆ ಇಳಿಕೆಯಲ್ಲಿದೆ — ಇಂದೇ ಮಾರಿ. ವಿಶ್ವಾಸಾರ್ಹತೆ: ${conf}/100.`;
    return `${kn} ಬೆಲೆ ಮುಂದಿನ ${days} ದಿನಗಳಲ್ಲಿ ₹${price}/ಕ್ವಿಂಟಲ್ ಸಮೀಪ ಸ್ಥಿರವಾಗಿರಲಿದೆ. ಮಾರುಕಟ್ಟೆ ಸಾಮಾನ್ಯ — ಆತುರ ಬೇಡ. ವಿಶ್ವಾಸಾರ್ಹತೆ: ${conf}/100.`;
  }
  if (action === "HOLD") return `${en} price is forecast to rise from ₹${current} to ₹${price} / quintal over the next ${days} days (+${pct}%). Market is bullish — do not sell now. Store in the government warehouse below and sell later for higher profit. Model confidence: ${conf}/100.`;
  if (action === "SELL") return `${en} price is forecast to drop from ₹${current} to ₹${price} / quintal over the next ${days} days (-${pct}%). Market is bearish — sell as soon as possible to avoid losses. Model confidence: ${conf}/100.`;
  return `${en} price is expected to stay near ₹${price} / quintal over the next ${days} days. Market is stable — no urgency to sell. Model confidence: ${conf}/100.`;
};
...
export default function PredictionTab({ profile, activeCrop }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {apiError && !loading && (
        <div style={{
          background: "rgba(184,120,10,0.09)", border: "1px solid rgba(184,120,10,0.25)",
          borderRadius: 10, padding: "10px 14px", fontSize: 11, color: "#B8780A"
        }}>⚠ {apiError}</div>
      )}
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <a href={`tel:${wh.phone}`} style={{
                      ...
                      <div>
                        <div style={{
                          fontSize: 9, color: G.muted, textTransform: "uppercase",
                  ...
                  {/* NWR tip banner */}
                  <div style={{
                    padding: "10px 16px", background: "rgba(27,107,53,0.06)",
                    display: "flex", alignItems: "center", gap: 10
                  }}>
