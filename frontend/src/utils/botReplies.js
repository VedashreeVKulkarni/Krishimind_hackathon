...
const RULES = [
  ...
  },
  {
    ...
    answer: (c) => `📈 Prices projected to peak around **Day ${Math.round(c.days * 0.75)}–${Math.round(c.days * 0.82)}** at ~₹${Math.round(c.pred * 1.04)} / quintal.`
  },
  {
    keys: ["price", "rate", "today", "current", "modal"],
    answer: (c) => `💰 Current modal: **₹${Math.round(c.base)} / quintal** at ${c.mandi}. Live from Agmarknet.`
  },
  {
  ...
  },
  {
    ...
    answer: () => `📋 Govt recently raised MSP by ₹200/quintal. MSP acts as a floor — actual market prices may trade above if demand stays strong.`
  },
];
